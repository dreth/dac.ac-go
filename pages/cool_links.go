...
func CoolLinksContentHTML(lang string) *elem.Element {
	...
	content := elem.Div(
		...
		elem.Div(
			attrs.Props{
				attrs.Class: "grid grid-cols-1 sm:grid-cols-2 gap-2 sm:py-2 pt-2",
			},
			elem.Div(
				attrs.Props{
					attrs.Class: "left-column rounded-lg bg-almost-black-2 p-4 h-dvh scroll-smooth overflow-y-auto",
				},
				elem.H2(
					HeadingProps,
					elem.Text("Wikipedia"),
			...
			elem.Div(
				attrs.Props{
					attrs.Class: "right-column pb-6 space-y-2 sm:h-dvh",
				},
				elem.Div(
					attrs.Props{
