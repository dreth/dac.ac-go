...
func ArticleHTMLStyling(mdConversion []byte) string {
	...
	doc.Find("img").Each(func(i int, s *goquery.Selection) {
		// Fetch the current src attribute
		src, exists := s.Attr("src")
		if exists {
			// Prepend the URL to the current src value
			newSrc := constants.ResourceWithCDN(src)
...
func ArticleList(amount int) []elem.Node {
	...
	for i := 0; i < len(listKeys); i++ {
		...
		articlesBlock = append(articlesBlock,
			elem.Div(
				attrs.Props{
					attrs.Class: "pb-3 sm:pb-5 border-l-4 border-almost-black-3  hover:border-almost-blue pl-2",
				},
				elem.P(
					attrs.Props{
