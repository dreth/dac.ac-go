...
func FooterLangSwitcher(lang string) *elem.Element {
	...
	return elem.A(
		attrs.Props{
			...
			attrs.Class:           Rt("inline-block no-underline text-links m-0 cursor-pointer hover:text-links-hover pl-5 transform xs:translate-y-px sm:translate-y-0.5  transform transition-transform duration-300 hover:scale-110 hover:shadow-lg"),
...
func Footer(lang string) *elem.Element {
	return elem.Footer(
		attrs.Props{
			attrs.Class: RtFooter("text-center text-text p-2 flex justify-center items-center fixed z-10 w-full bottom-0 bg-almost-black h-11  bg-opacity-60"),
		},
		elem.Div(
			nil,
	...
	return elem.A(
		...
		elem.Img(
			attrs.Props{
				...
				attrs.Alt:   network,
				attrs.Title: network,
				attrs.Class: RtFooter("w-5 h-auto inline-block align-middle ml-4 hover:text-gray-500  transform transition-transform duration-300 hover:scale-110 hover:shadow-lg"),
			},
		),
	)
}
